...
// if the localstorage has any value then it will be get converted into an object(Array) if not then only we will return an array but it is an empty one
...
function increaseItem(product, cartItemDOM) {
  cart.forEach((cartItem) => {
    if (cartItem.name === product.name) {
      cartItemDOM.querySelector(".cart-item-quantity").innerText =
        ++cartItem.quantity;
      cartItemDOM
        .querySelector('[data-action="DECREASE_ITEM"]')
...
function countCartTotal() {
  ...
  document.querySelector('[data-action="CHECKOUT"]').innerText =
