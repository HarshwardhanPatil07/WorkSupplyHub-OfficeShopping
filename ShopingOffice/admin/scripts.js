...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function showGroup(index) {
    hideAllGroups();
    const groupsToShow = partnerGroups.slice(index, index + 3);
  ...
  function slideToNext() {
    currentIndex = (currentIndex + 3) % partnerGroups.length;
    showGroup(currentIndex);
  }

  function slideToPrevious() {
    currentIndex = (currentIndex - 3 + partnerGroups.length) % partnerGroups.length;
    showGroup(currentIndex);
  }
  ...
  prevArrow.addEventListener('click', slideToPrevious);
  nextArrow.addEventListener('click', slideToNext);

  setInterval(slideToNext, 3000); // Change partner groups every 3 seconds (adjust as needed)
});
